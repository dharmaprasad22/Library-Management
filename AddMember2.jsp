...
<html>
    ...
    <div class="content">
        ...
        Connection    con = DriverManager.getConnection("jdbc:mysql://localhost:3306/library","root","root");
            ...
            PreparedStatement ps = con.prepareStatement("select * from member where MemID='" + memID + "';");
            ResultSet rs = ps.executeQuery();
            if (rs.next() == true) {
                                    %>
                            <br><h1>Unable to Save, The Given Member ID already exists in the Database !!</h1><br>
                            <%
            } else {
                ps = con.prepareStatement("insert into member(MemID,name,sem,stream,DateOfJoin,DateOfExpire,phoneno,Email) values('" + memID + "','" + name + "','" + sem + "','" + stream + "','" + doj + "','" + doe + "','" + phoneno + "','" + email + "');");
                int i = ps.executeUpdate();
                ps=con.prepareStatement("insert into login(username,password) values('" + memID + "','" + memID + "');");
                i = ps.executeUpdate();
                            %>
                            <br><h1>New Member is Successfully Added.</h1><br>
                            <%
            }
                            %>
